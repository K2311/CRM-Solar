                    ...
                    <div style="grid-column: span 2; border: 1px solid var(--border); padding: 1.5rem; border-radius: 0.5rem; background: var(--bg-surface);">
                        <div style="margin-bottom: 1rem; display: flex; gap: 1.5rem; align-items: center; border-bottom: 1px solid var(--border); padding-bottom: 1rem;">
                            ...
                            <label style="cursor: pointer; display: flex; align-items: center; gap: 0.5rem;">
                                <input type="radio" name="customer_type" value="new" {{ old('customer_type', 'new') === 'new' ? 'checked' : '' }} onchange="document.getElementById('new_customer_fields').style.display='grid'; document.getElementById('existing_customer_fields').style.display='none';"> 
                                Quick Add New Prospect
                            </label>
                            <label style="cursor: pointer; display: flex; align-items: center; gap: 0.5rem;">
                                <input type="radio" name="customer_type" value="existing" {{ old('customer_type') === 'existing' ? 'checked' : '' }} onchange="document.getElementById('new_customer_fields').style.display='none'; document.getElementById('existing_customer_fields').style.display='block';"> 
                                Select Existing Customer
                            </label>
                        </div>

                        <div id="new_customer_fields" style="display: {{ old('customer_type', 'new') === 'new' ? 'grid' : 'none' }}; grid-template-columns: 1fr 1fr 1fr; gap: 1rem;">
                            <div>
                        ...
                        <div id="existing_customer_fields" style="display: {{ old('customer_type') === 'existing' ? 'block' : 'none' }};">
                            <label class="form-label">Select Customer <span style="color: #ef4444;">*</span></label>
                            ...
                            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 0.75rem; max-height: 200px; overflow-y: auto; padding: 1rem; border: 1px solid var(--border); border-radius: 0.5rem; background: var(--bg-surface);">
                                @foreach($products as $product)
                                    <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer; padding: 0.5rem; border-radius: 0.25rem; transition: background 0.2s;" onmouseover="this.style.background='rgba(0,0,0,0.02)'" onmouseout="this.style.background='transparent'">
                                        <input type="checkbox" name="product_ids[]" value="{{ $product->id }}" {{ collect(old('product_ids'))->contains($product->id) ? 'checked' : '' }} style="accent-color: var(--primary); width: 1.1rem; height: 1.1rem;">
