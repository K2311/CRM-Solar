<x-app-layout title="Edit Template">
    <div style="max-width: 1200px; margin: 0 auto;" x-data="templatePreview()">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
            <div>
                <h1 style="font-size: 1.875rem; font-weight: 800; margin-bottom: 0.5rem;">Edit Template</h1>
                <p style="color: var(--text-muted);">Modify your message template details and see live changes.</p>
            </div>
            ...
            <div class="card">
                <form action="{{ route('templates.update', $template) }}" method="POST">
                    ...
                    <div style="margin-bottom: 1.5rem;" x-show="channel === 'email'">
                        ...
                        <input type="text" name="subject" class="form-control" x-model="subject" value="{{ old('subject', $template->subject) }}">
                    </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('templatePreview', () => ({
                channel: @json(old('channel', $template->channel)),
                subject: @json(old('subject', $template->subject)),
