<x-app-layout title="Edit Post">
    <div style="max-width: 1200px; margin: 0 auto;" x-data="socialEditor()">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
            <div>
                <h1 style="font-size: 1.875rem; font-weight: 800; margin-bottom: 0.5rem;">Edit Social Media Post</h1>
                <p style="color: var(--text-muted);">Update your scheduled or draft post.</p>
            </div>
            ...
            <div class="card">
                <form action="{{ route('social.update', $post) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <template x-if="mediaPreview">
                            <div style="display: flex; align-items: stretch; min-height: 110px;">
                                ...
                                <div style="flex:1; padding: 1rem 1rem 1rem 1.25rem; display:flex; flex-direction:column; justify-content:center; gap:0.4rem;">
                                    <div>
                                        <span x-text="mediaFileType || 'FILE'"
                                    ...
                                    <div x-text="mediaFileName || 'Current Uploaded Media'"
                    ...
                    <div style="display: flex; gap: 1rem; justify-content: flex-end; border-top: 1px solid var(--border); padding-top: 1.5rem;">
                        ...
                        <button type="submit" class="btn btn-primary" x-html="scheduledAt ? '<i class=\'bi bi-calendar-check\'></i> Save Scheduled Post' : '<i class=\'bi bi-send\'></i> Save & Publish Now'"></button>
                    </div>
                </form>
            </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('socialEditor', () => ({
                platform:      '{{ old('platform', $post->platform) }}',
                postType:      '{{ old('post_type', $post->post_type) }}',
                content:       {!! json_encode(old('content', $post->content)) !!},
                scheduledAt:   '{{ old('scheduled_at', $post->scheduled_at ? $post->scheduled_at->format('Y-m-d\TH:i') : '') }}',
                mediaPreview:  '{{ !empty($post->media_urls) ? $post->media_urls[0] : null }}',
                mediaFileName: '',
                mediaFileSize: '',
                mediaFileType: '',
                dragOver:      false,
